...
if ($_SERVER["REQUEST_METHOD"] == "GET") {
    ...
    $summary = $_GET["summary"]; // Add this line to get the selected category

    try {
        // Update the category column in the profile table
        $updateSql = "UPDATE profile SET Summary = ? WHERE username = ?";
        $updateStmt = $conn->prepare($updateSql);

        $updateStmt->bind_param("ss", $summary, $userEmail);
        $updateStmt->execute();
