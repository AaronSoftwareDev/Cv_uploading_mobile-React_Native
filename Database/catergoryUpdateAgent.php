...
if ($_SERVER["REQUEST_METHOD"] == "GET") {
    ...
    $newItemsEmail = $_GET["newItemsEmail"];
    $registrationDate = date('d-M-Y');
    // Add this line to get the newItemsEmail

    try {
        // Update the category column in the profile table
        $updateSql = "UPDATE profile SET category = ?, agent_email = ?, reviewed_date = ?,  review_status = 'Viewed' WHERE username = ?";

        $updateStmt = $conn->prepare($updateSql);

        $updateStmt->bind_param("ssss", $selectedCategory, $newItemsEmail,  $registrationDate, $userEmail);
        $updateStmt->execute();
